...
result=$(curl -f -s -X POST "$SERVER/bjcp/_search?pretty" -H 'Content-Type: application/json' -d $'{
  "query": {
    "script_score": {
      "query": {
        "exists": {
...
}' | jq  -r --compact-output  ".hits.hits[] | { "score": ._score, "id": ._id, "subcategory": ._source.subcategory, "vector": ._source.vector,}"  | paste -sd "," -)

echo "{\"ref\": \"$BEER\", \"result\": [$result]}"
